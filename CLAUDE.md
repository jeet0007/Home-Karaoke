...
python app.py                            # run the app (127.0.0.1:3000 by default - not 5000, which macOS's AirPlay Receiver squats on; APP_HOST/APP_PORT to override)
...
templates/ static/  the vanilla-JS TV player + phone GUIs (static/player/grading.js runs live grading client-side via the WASM module inside static/grading-worklet.js, an AudioWorklet, falling back to the /grade WebSocket only if WebAssembly is unavailable; static/player/singer-assist.js toggles the isolated vocal stem in as a guide track) - see "TV + phone room pairing" below; `/` redirects straight to `/tv`, there is no standalone single-device search page anymore
...
### TV + phone room pairing

The app is a "TV as display, phone as mic + remote" experience (`core/rooms.py`): a TV boots into `templates/tv_lobby.html` (`/` redirects here), creates a room (`POST /room/create`) and shows a fixed QR code (`GET /room/<code>/qr.svg`, rendered via `core/qrcode_svg.py` straight from `qrcode`'s boolean module grid — no Pillow dependency, no client-side QR library needed either, since the phone screen is just the OS camera viewfinder scanning a URL). Phones open that URL (`GET /join/<code>` → `templates/phone_join.html`) to join. There is no single-device fallback UI — picking and queuing songs happens entirely from a joined phone; the TV only ever plays what the room's queue hands it (`templates/player.html`, extended with a `?room=` mode — see below).

Realtime sync between the TV and its phones is one WebSocket route, `/room-ws` (reuses the existing flask-sock/simple-websocket dependency `/grade` already brought in — no new pub/sub dependency). `RoomRegistry` in `core/rooms.py` is an **in-memory, single-process** room→connections registry, deliberately not persisted to SQLite: a room has no meaning once its TV disconnects, matching the `_live_select_cache` precedent and the project's no-broker stance. This means the feature does not work if the app is ever run behind multiple WSGI worker processes (it isn't today). See `core/rooms.py`'s module docstring for the thread-safety design (per-connection send locks, registry lock never held across `.send()` calls).

**LAN IP requirement**: the QR code's payload must be a URL a phone on the same network can reach, so `APP_HOST=127.0.0.1` (the default) won't work for this feature — set `APP_HOST=0.0.0.0` (or a specific LAN IP) to actually serve requests from a phone. Separately, the QR *payload* itself needs the server's real LAN IP, auto-detected via `app.py`'s `_detect_lan_ip()` (a UDP-socket route lookup, no packet actually sent); override with `APP_LAN_HOST` when auto-detection guesses wrong — ambiguous multi-NIC hosts, VPN interfaces, or Docker's default bridge network, where a container's own view of "LAN" isn't the host's (the `docker-compose.yml` `web` service will need `APP_LAN_HOST` set explicitly, or `network_mode: host`, to hand out a phone-reachable address).

**Queue playback (TV Now Playing + phone Now Singing)**: the sing-queue itself lives entirely in `core/rooms.py` from the start (`enqueue_song`/`advance_queue`/etc.), but nothing plays it until `static/tv/lobby.js` notices the queue has a song and `now_playing` is still empty, and sends `{"type": "advance-queue"}` itself — the TV, not the server, decides when to start (mirrors `route_playback_control`'s "TV is sole source of truth" stance). Once `now_playing` is non-null, the lobby navigates the browser to `GET /player?...&room=<code>&entry=<entry_id>` — the *same* `templates/player.html` single-device players use, extended with a small always-hidden-unless-`?room=` overlay (room code + up next) and `static/player/room-broadcast.js`, which attaches as that room's TV, broadcasts `playback-position` on a timer, and applies `playback-control` from phones through the exact same `togglePlayback`/`restartPlayback` the on-screen buttons call — no parallel control path. `static/player/main.js`'s song-loading logic was refactored into a parameterized `loadSongByIdentity()` for this: called once at boot for the URL's own song, and again by `room-broadcast.js` every time the room advances to a new queue entry, without a page reload. A song ending, or a phone's "Skip", both just send `{"type": "advance-queue"}` back to the server; when that empties the queue (`now_playing: null`), the TV navigates back to `GET /tv?code=<code>` — the `code` query param makes `/tv` *reattach* to the existing room (skipping `POST /room/create`) instead of abandoning it for a fresh one, which is what makes leaving-and-returning to the lobby safe.

On the phone side, `GET /room/<code>/now-singing` (`templates/phone_now_singing.html` + `static/phone/now-singing.js`) is the synced-lyrics + remote-control screen — a separate page/socket, like `/join/<code>`, rather than a fourth tab in `phone_home.html`'s Home/Search/Queue SPA. It reuses `static/player/lyrics.js` completely unmodified (`updateCurrentLyric(nowMs)`/`resync(nowMs)` already took an explicit position argument, not a DOM `<audio>` read). Since the phone has no local audio to derive a position from, `static/shared/clock.js`'s `createRemoteClock()` extrapolates between the TV's periodic `playback-position` broadcasts using `performance.now()` deltas, so lyric highlighting doesn't stutter between updates. `templates/phone_home.html` shows a persistent "Now singing" banner (visible across all three tabs) linking to this screen whenever `now_playing` is non-null.

**Mic scoring runs on the phone, not the TV** (the "local grading only" decision: no audio is ever streamed between devices — only scores). The Now Singing screen's "Start Mic" button calls the *same* `ensureGradingStarted()` the single-device player already used, from `static/player/grading.js` — now parameterized (`options.getPositionMs`/`options.isPlaying`/`options.onScoreUpdate`, all optional and defaulting to the original TV-only behavior) so the phone can hand it a `RemoteClock`-backed position instead of reading a local `<audio>` element, and forward each score over `/room-ws` as `{"type": "score-update", ...}` for `core/rooms.py`'s `route_score_update` to relay to the TV. `grading.js` pulls in `note-guide.js` as a transitive dependency, which does `noteGuideCanvas.getContext('2d')` unconditionally at module-load time — `phone_now_singing.html` includes a hidden, otherwise-unused `#note-guide-canvas` purely so that lookup doesn't throw on a page with no visible note guide. The voice-assist blend slider on the same screen sends `{"type": "voice-assist-volume", "volume": ...}`; `room-broadcast.js` routes it into a new `setVolumeRemote()` export on `static/player/singer-assist.js`, which also turns the TV's stem on if it wasn't already (a nonzero remote blend implies intent) rather than requiring a separate remote toggle.
...
GET  /tv (TV lobby boot; ?code= reattaches to an existing room instead of creating one), POST /room/create, GET /room/<code>/state, GET /room/<code>/qr.svg, GET /join/<code>, GET /room/<code> (phone app shell), GET /room/<code>/now-singing (phone lyrics + remote control), GET /room-suggestions
/room-ws  WebSocket — TV+phone room pairing/sync, see "TV + phone room pairing" above
